...
namespace ImageResizing
{
    ...
    class ContentAwareResize
    {
        ...
        private static int pixel_Energy(Color A, Color B, Color C, Color D, Color E, Color F, Color G, Color H, Color I)
        {
           ...
           // return Math.Abs(Ex) + Math.Abs(Ey);
        ...
        private static void Ver_C(int start, int first_pixel_index, Color[,] image, int[,] energy, int[,] C, int[,] map, int Width, int Height)
        {
           ...
           // int[,] case_1 = { { -1, -1 }, { -1, 1 }, { 0, -1 }, { 0, 1 } };
            ...
            int begin = Math.Max(first_pixel_index -1,0), end = Math.Min(first_pixel_index,Width-1);

            for (int i = Math.Max(start-1,1); i < Height+1; ++i)
            {
                ...
                for (int j = begin; j <= end; ++j)
                {
                    ...
                    for (int k = -1; k <= 1; ++k)
                    {
                        ...
                        for (int m = 0; m < 2; ++m)
                        {
                            before_erase += //getEnergy(i + case_1[m, 0], j + case_1[m, 1] + k, Width, Height, energy);
                                + getEnergy(i + case_1[m , 0], j + case_1[m , 1], Width, Height, energy);

                            after_erase += //forward_energy(true, i + case_1[m, 0], j + case_1[m, 1] + k, i, j, i - 1, j + k, Width, Height, image);
                                + forward_energy(true, i + case_1[m , 0], j + case_1[m , 1], i, j, i - 1, j + k, Width, Height, image);
                        }
                    ...
                    if (j == begin && C[i, j] != tmp)
                    {
                        begin = Math.Max(0, begin - 1);
                    }
                    else if (j == end && C[i, j] != tmp)
                    {
                        end = Math.Min(end + 1, Width - 1);
                    }
                }
            }
        }
        private static void Hor_C(int start, int first_pixel_index, Color[,] image, int[,] energy, int[,] C, int[,] map, int Width, int Height)
        {
            ...
            const int INF = 2147483647;
            int[,] case_1 = { { 0, -1 }, { 0, 1 } };
            int begin = Math.Max(first_pixel_index - 1, 0), end = Math.Min(first_pixel_index, Height - 1);

            for (int j = Math.Max(start - 1, 1); j < Width+1; ++j)
            {
                //if (start == 1)
                //{
                //    int length = (2 * j) + 1;
                ...
                for (int i = end; i >= begin; --i)
                {
                    ...
                    for (int k = -1; k <= 1; ++k)
                    {
                        ...
                        for (int m = 0; m < 2; ++m)
                        {
                            before_erase += //getEnergy(i + case_1[m, 1] + k, j + case_1[m, 0], Width, Height, energy)
                                + getEnergy(i + case_1[m , 1], j + case_1[m , 0], Width, Height, energy);

                            after_erase += //forward_energy(false, i + case_1[m, 1] + k, j + case_1[m, 0], i, j, i + k, j - 1, Width, Height, image)
                                + forward_energy(false, i + case_1[m , 1], j + case_1[m , 0], i, j, i + k, j - 1, Width, Height, image);
                        }
                    ...
                    if (i == begin && C[i, j] != tmp)
                    {
                        begin = Math.Max(0, begin - 1);
                    }
                    else if (i == end && C[i, j] != tmp)
                    {
                        end = Math.Min(end + 1, Height - 1);
                    }
                }
            }
        }
        ...
        public static Color[,] seamCarving_resize(Color[,] Image, int height_resize, int width_resize, BackgroundWorker bw)
        {
            ...
            int[,] C_ver = new int[Height+1, Width];
            int[,] map_ver = new int[Height+1, Width];
            int[,] C_hor = new int[Height, Width+1];
            int[,] map_hor = new int[Height, Width+1];
